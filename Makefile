...
service: $(SYSTEM)/osfooler.service $(SYSTEM)/osfooler.service.d $(SYSTEM)/osfooler.service.d/osfooler.conf | $(VENV)
	chown root:root $^
	chmod 644 $^
...
$(VENV):
	$(PYTHON) -m venv $@
	sh -c ". $@/bin/activate; python3 -m pip install -r requirements.txt"
